...
out_dir <- "output"
out_dir_basic_df <- "processed_data_WAT"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(out_dir_basic_df, recursive = TRUE, showWarnings = FALSE)
...
write_xlsx(expr_matrix, file.path(out_dir_basic_df,"expression_matrix_WAT.xslx"))
```
...
ggsave(file.path(out_dir,"GO Biological Process Enrichment Unique DEG per Comparison - Neuronal Regulation Related Terms.png"),neuronal_regulation_go, height =9, width = 13)
```
```{r}
# Filter neuronal GO terms
neuronal_terms <- sig_ego %>%
  filter(grepl("synap|neuro", Description, ignore.case = TRUE))

# Split genes into a list
gene_list <- strsplit(neuronal_terms$geneID, split = "/")

# Get all unique genes
all_genes <- unique(unlist(gene_list))

# Create binary matrix: GO terms (rows) x genes (columns)
go_gene_matrix <- t(sapply(gene_list, function(x) all_genes %in% x))
rownames(go_gene_matrix) <- neuronal_terms$Description
colnames(go_gene_matrix) <- all_genes

# Convert logical to numeric
go_gene_matrix <- 1 * go_gene_matrix

gene_counts_go <- colSums(go_gene_matrix)
top_genes <- names(sort(gene_counts_go, decreasing = TRUE)[1:40])

pheat_go<- pheatmap(go_gene_matrix[, top_genes],
                   cluster_rows = TRUE,
                   cluster_cols = TRUE,
                   ...
                   fontsize_col = 9,
                   angle_col = 45)
ggsave(file.path(out_dir,"GO_WAT_BP_genes.png"),pheat_go, height =9, width = 13)
```
...
```{r tag enzymes without Dgat1}
tag_enzymes_without_dgat1 <- c("Dgat2", "Plpp3", "Agpat2", 
                 "Lpcat3", "Lpgat1")

expr_subset <- expr_matrix[rownames(expr_matrix) %in% tag_enzymes_without_dgat1, ]
...
expr_long_melt$Expression <- as.numeric(expr_long_melt$Expression)
...
ggsave(file.path(out_dir,"TAG synthesis enzymes without Dgat1.png"), p_scaled,height = 6, width = 11)
```
