...
keywords <- c(
  "leptin", "insulin", "energy", "feeding", "appetite",
  "neuropeptide",
  "JAK", "STAT", 
...
p_up <- ggplot(go_up_plot, aes(x = Description, y = negLogP)) +
  geom_col(fill = col_up, width = 0.7) +
  coord_flip() +
  labs(
    title = "Upregulated Biological Processes",
    subtitle = "Lactation HFHS vs Chow HFHS (Hypothalamus)",
    x = NULL,
...
p_down <- ggplot(go_down_plot, aes(x = Description, y = negLogP)) +
  geom_col(fill = col_down, width = 0.7) +
  coord_flip() +
  labs(
    title = "Downregulated Biological Processes",
    subtitle = "Lactation HFHS vs Chow HFHS (Hypothalamus)",
    x = NULL,
    y = expression(-log[10]("adjusted p-value"))
  ) +
...
p_bp <- dotplot(go_bp, showCategory = 20) +
  ggtitle("GO Biological Processes (DEGs)")

p_cc <- dotplot(go_cc, showCategory = 20) +
  ggtitle("GO Cellular Component (DEGs)")

p_mf <- dotplot(go_mf, showCategory = 20) +
  ggtitle("GO Molecular Function (DEGs)")
p_bp
ggsave(
...
ggsave(
  filename = file.path(out_dir, "GO_CC_dotplot.png"),
  plot = p_cc,
  width = 6,
  height = 8,
  dpi = 300
...
# Get KEGG gene IDs (ENTREZ IDs) for mouse leptin signaling pathway
kegg_leptin <- keggGet("mmu04920")[[1]]$GENE

# Extract Entrez IDs only (even indices)
entrez_ids <- kegg_leptin[seq(1, length(kegg_leptin), 2)]
